...
if ! docker compose -f "$COMPOSE_FILE" run --rm "$APP_SERVICE" \
  python manage.py migrate --noinput; then
  echo "Migration failed; application was not replaced."
  exit 1
fi
...
rollback "$OLD_IMAGE" || \
  echo "Automatic application-image rollback was not possible."
